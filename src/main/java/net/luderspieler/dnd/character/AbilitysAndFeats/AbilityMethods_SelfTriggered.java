...
public class AbilityMethods_SelfTriggered {
     ...
     * over resistance.
     */
    private void handleDamageTypeResistances(ServerPlayer player, LivingIncomingDamageEvent event) {
        ...
        boolean resistant = AbilityUtils.hasAbility(player, Ability.valueOf(typeKey + "_DAMAGE_RESISTANCE"))
                || typeKey.equals(GeneralDataUtils.get(vars, "DraconicAncestryDamageType", ""));
        if (resistant) {
            event.setAmount(event.getAmount() / 2.0f);
        }
    }
