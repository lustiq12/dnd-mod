...
@EventBusSubscriber
public class TemplateAnimSyncProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, DamageSource damagesource, Entity entity, Entity immediatesourceentity, double amount) {
		if (damagesource == null || entity == null || immediatesourceentity == null)
			return;
		if (immediatesourceentity instanceof TemplateMobEntity && amount == 100) {
			if (event instanceof ICancellableEvent _cancellable) {
				_cancellable.setCanceled(true);
			}
			if (immediatesourceentity instanceof TemplateMobEntity _datEntSetI)
				_datEntSetI.getEntityData().set(TemplateMobEntity.DATA_cooldown, 15);
			if (immediatesourceentity instanceof TemplateMobEntity _datEntSetS)
				_datEntSetS.getEntityData().set(TemplateMobEntity.DATA_anim, "attack");
