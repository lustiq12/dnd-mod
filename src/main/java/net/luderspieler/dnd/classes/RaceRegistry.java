...
public class RaceRegistry {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  ATTRIBUTE MAPS  (main races only — added on top of class attrs)
    // ═══════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════
    //  STARTER ITEMS  (main races only)
    // ═══════════════════════════════════════════════════════════════

    private static final List<ItemStack> AASIMAR_ITEMS    = List.of(new ItemStack(Items.GOLDEN_APPLE), new ItemStack(Items.BREAD, 4));
    private static final List<ItemStack> DRAGONBORN_ITEMS = List.of(new ItemStack(Items.IRON_CHESTPLATE), new ItemStack(Items.COOKED_BEEF, 4));
    private static final List<ItemStack> DWARF_ITEMS      = List.of(new ItemStack(Items.IRON_PICKAXE), new ItemStack(Items.IRON_INGOT, 3));
    private static final List<ItemStack> ELF_ITEMS        = List.of(new ItemStack(Items.BOW), new ItemStack(Items.ARROW, 16));
    private static final List<ItemStack> GNOME_ITEMS      = List.of(new ItemStack(Items.GOLD_NUGGET, 5), new ItemStack(Items.BREAD, 4));
    private static final List<ItemStack> GOLIATH_ITEMS    = List.of(new ItemStack(Items.IRON_AXE), new ItemStack(Items.COOKED_BEEF, 6));
    private static final List<ItemStack> HALFLING_ITEMS   = List.of(new ItemStack(Items.BREAD, 10), new ItemStack(Items.LEATHER_BOOTS));
    private static final List<ItemStack> HUMAN_ITEMS      = List.of(new ItemStack(Items.BREAD, 6));
    private static final List<ItemStack> ORC_ITEMS        = List.of(new ItemStack(Items.IRON_AXE), new ItemStack(Items.COOKED_BEEF, 6));
    private static final List<ItemStack> TIEFLING_ITEMS   = List.of(new ItemStack(Items.BLAZE_ROD), new ItemStack(Items.BREAD, 4));
    ...
    // ═══════════════════════════════════════════════════════════════
    //  ABILITY LINES  (subraces — no stats, no items)
    // ═══════════════════════════════════════════════════════════════
    ...
    static {
        RACES.add(new RaceDefinition("aasimar",    "Aasimar",    "Touched by the divine, born to serve the light.",     AASIMAR_ATTRS,    AASIMAR_AB,    AASIMAR_ITEMS));
        SUBRACES.add(new SubraceDefinition("celestial_revelation", "aasimar",    "Celestial Revelation", "Unleashes the full power of divine heritage.",    CELESTIAL_REVELATION_AB));

        RACES.add(new RaceDefinition("dragonborn", "Dragonborn", "Proud dragon-blooded warriors.",                      DRAGONBORN_ATTRS, DRAGONBORN_AB, DRAGONBORN_ITEMS));
        SUBRACES.add(new SubraceDefinition("chromatic_dragonborn",  "dragonborn", "Chromatic Dragonborn", "Descended from the chromatic dragons.",           CHROMATIC_DRAGONBORN_AB));
        SUBRACES.add(new SubraceDefinition("metallic_dragonborn",   "dragonborn", "Metallic Dragonborn",  "Noble lineage of the metallic dragons.",          METALLIC_DRAGONBORN_AB));
        SUBRACES.add(new SubraceDefinition("gem_dragonborn",        "dragonborn", "Gem Dragonborn",       "Descended from the rare gem dragons.",            GEM_DRAGONBORN_AB));

        RACES.add(new RaceDefinition("dwarf",      "Dwarf",      "Stout and resilient, masters of stone and steel.",   DWARF_ATTRS,      DWARF_AB,      DWARF_ITEMS));
        SUBRACES.add(new SubraceDefinition("hill_dwarf",            "dwarf",      "Hill Dwarf",           "Hardy and wise, blessed with great vitality.",    HILL_DWARF_AB));
        SUBRACES.add(new SubraceDefinition("mountain_dwarf",        "dwarf",      "Mountain Dwarf",       "Trained for war, clad in the finest armor.",      MOUNTAIN_DWARF_AB));

        RACES.add(new RaceDefinition("elf",        "Elf",        "Ancient, graceful and deeply tied to magic.",        ELF_ATTRS,        ELF_AB,        ELF_ITEMS));
        SUBRACES.add(new SubraceDefinition("drow",                  "elf",        "Drow",                 "Dark elves from the depths of the Underdark.",    DROW_AB));
        SUBRACES.add(new SubraceDefinition("high_elf",              "elf",        "High Elf",             "Devoted to magic and lore of ancient times.",     HIGH_ELF_AB));
        SUBRACES.add(new SubraceDefinition("wood_elf",              "elf",        "Wood Elf",             "Swift hunters of the deep forest.",               WOOD_ELF_AB));

        RACES.add(new RaceDefinition("gnome",      "Gnome",      "Curious and inventive, full of wonder.",             GNOME_ATTRS,      GNOME_AB,      GNOME_ITEMS));
        SUBRACES.add(new SubraceDefinition("forest_gnome",          "gnome",      "Forest Gnome",         "Friends of animals and nature's secrets.",        FOREST_GNOME_AB));
        SUBRACES.add(new SubraceDefinition("rock_gnome",            "gnome",      "Rock Gnome",           "Tinkerers and craftsmen of the earth.",           ROCK_GNOME_AB));

        RACES.add(new RaceDefinition("goliath",    "Goliath",    "Towering warriors born of the mountains.",           GOLIATH_ATTRS,    GOLIATH_AB,    GOLIATH_ITEMS));
        SUBRACES.add(new SubraceDefinition("cloud_giant",           "goliath",    "Cloud Giant Ancestry", "Born of the sky, blessed with grace and wind.",   CLOUD_GIANT_AB));
        SUBRACES.add(new SubraceDefinition("fire_giant",            "goliath",    "Fire Giant Ancestry",  "Forged in flame, relentless in battle.",          FIRE_GIANT_AB));
        SUBRACES.add(new SubraceDefinition("frost_giant",           "goliath",    "Frost Giant Ancestry", "Born of the frozen north, unyielding as ice.",    FROST_GIANT_AB));
        SUBRACES.add(new SubraceDefinition("hill_giant",            "goliath",    "Hill Giant Ancestry",  "Massive and enduring, born to weather any storm.",HILL_GIANT_AB));
        SUBRACES.add(new SubraceDefinition("stone_giant",           "goliath",    "Stone Giant Ancestry", "Still as stone, armored like the earth itself.",  STONE_GIANT_AB));
        SUBRACES.add(new SubraceDefinition("storm_giant",           "goliath",    "Storm Giant Ancestry", "Commands the power of thunder and lightning.",    STORM_GIANT_AB));

        RACES.add(new RaceDefinition("halfling",   "Halfling",   "Small but extraordinarily lucky.",                   HALFLING_ATTRS,   HALFLING_AB,   HALFLING_ITEMS));
        SUBRACES.add(new SubraceDefinition("lightfoot",             "halfling",   "Lightfoot Halfling",   "Nimble and stealthy, with a natural silver tongue.",LIGHTFOOT_AB));
        SUBRACES.add(new SubraceDefinition("stout_halfling",        "halfling",   "Stout Halfling",       "Tougher than they look, with dwarven blood.",     STOUT_AB));

        RACES.add(new RaceDefinition("human",      "Human",      "Adaptable and ambitious, found everywhere.",         HUMAN_ATTRS,      HUMAN_AB,      HUMAN_ITEMS));
        SUBRACES.add(new SubraceDefinition("versatile_human",       "human",      "Versatile Human",      "Masters of adaptability, excelling in any field.",VERSATILE_HUMAN_AB));

        RACES.add(new RaceDefinition("orc",        "Orc",        "Powerful and fierce warriors of the wilds.",         ORC_ATTRS,        ORC_AB,        ORC_ITEMS));
        SUBRACES.add(new SubraceDefinition("adrenaline_rush",       "orc",        "Adrenaline Rush",      "Pushes beyond limits in the heat of battle.",     ADRENALINE_RUSH_AB));

        RACES.add(new RaceDefinition("tiefling",   "Tiefling",   "Bearing infernal heritage, often mistrusted.",       TIEFLING_ATTRS,   TIEFLING_AB,   TIEFLING_ITEMS));
        SUBRACES.add(new SubraceDefinition("abyssal_tiefling",      "tiefling",   "Abyssal Tiefling",     "Touched by demon blood, chaotic and swift.",      ABYSSAL_TIEFLING_AB));
        SUBRACES.add(new SubraceDefinition("chthonic_tiefling",     "tiefling",   "Chthonic Tiefling",    "Marked by the deepest underworld forces.",        CHTHONIC_TIEFLING_AB));
        SUBRACES.add(new SubraceDefinition("infernal_tiefling",     "tiefling",   "Infernal Tiefling",    "Descended from devils, burning with dark power.",  INFERNAL_TIEFLING_AB));
    }
    ...
    public static List<SubraceDefinition> getSubracesFor(String parentId) {
        ...
        for (SubraceDefinition s : SUBRACES)
            if (s.getParentRaceId().equals(parentId)) out.add(s);
        return out;
    }

    public static RaceDefinition getRace(String id) {
        for (RaceDefinition r : RACES) if (r.getId().equals(id)) return r;
        return null;
    }

    public static SubraceDefinition getSubrace(String id) {
        for (SubraceDefinition s : SUBRACES) if (s.getId().equals(id)) return s;
        return null;
    }
}