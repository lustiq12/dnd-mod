...
public class ClassRegistry {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  STARTER ITEMS  (main classes only)
    // ═══════════════════════════════════════════════════════════════
    ...
    private static final List<ItemStack> FIGHTER_ITEMS   = List.of(new ItemStack(Items.IRON_SWORD), new ItemStack(Items.IRON_CHESTPLATE), new ItemStack(Items.IRON_HELMET));
    ...
    private static final List<ItemStack> PALADIN_ITEMS   = List.of(new ItemStack(Items.IRON_SWORD), new ItemStack(Items.IRON_CHESTPLATE), new ItemStack(Items.IRON_HELMET));
    ...
    // ═══════════════════════════════════════════════════════════════
    //  ABILITY LINES  (subclasses — no stats, no items)
    // ═══════════════════════════════════════════════════════════════
    ...
    static {
        CLASSES.add(new ClassDefinition("barbarian", "Barbarian", "A fierce warrior driven by primal rage.",           BARBARIAN_ATTRS, BARBARIAN_AB, BARBARIAN_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("berserker",       "barbarian", "Path of the Berserker",       "Channels rage into devastating attacks.",           BERSERKER_AB));
        SUBCLASSES.add(new SubclassDefinition("wild_heart",      "barbarian", "Path of the Wild Heart",      "Draws on the spirits of beasts.",                   WILD_HEART_AB));
        SUBCLASSES.add(new SubclassDefinition("world_tree",      "barbarian", "Path of the World Tree",      "Taps into the cosmic power of the World Tree.",     WORLD_TREE_AB));
        SUBCLASSES.add(new SubclassDefinition("zealot",          "barbarian", "Path of the Zealot",          "Infuses rage with divine fury.",                    ZEALOT_AB));

        CLASSES.add(new ClassDefinition("bard",      "Bard",      "A master of music, magic, and inspiration.",        BARD_ATTRS,      BARD_AB,      BARD_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("college_dance",   "bard",      "College of Dance",            "Uses movement as a spellcasting focus.",            COLLEGE_DANCE_AB));
        SUBCLASSES.add(new SubclassDefinition("college_glamour", "bard",      "College of Glamour",          "Weaves enchantments of fey-touched beauty.",        COLLEGE_GLAMOUR_AB));
        SUBCLASSES.add(new SubclassDefinition("college_lore",    "bard",      "College of Lore",             "Collects knowledge from every discipline.",         COLLEGE_LORE_AB));
        SUBCLASSES.add(new SubclassDefinition("college_valor",   "bard",      "College of Valor",            "Combines combat prowess with bardic inspiration.",  COLLEGE_VALOR_AB));

        CLASSES.add(new ClassDefinition("cleric",    "Cleric",    "A divine spellcaster empowered by their deity.",    CLERIC_ATTRS,    CLERIC_AB,    CLERIC_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("life_domain",     "cleric",    "Life Domain",                 "Devoted to preserving and restoring life.",         LIFE_DOMAIN_AB));
        SUBCLASSES.add(new SubclassDefinition("light_domain",    "cleric",    "Light Domain",                "Channels the power of fire and radiance.",          LIGHT_DOMAIN_AB));
        SUBCLASSES.add(new SubclassDefinition("trickery_domain", "cleric",    "Trickery Domain",             "A deity of deception blesses this cleric.",         TRICKERY_DOMAIN_AB));
        SUBCLASSES.add(new SubclassDefinition("war_domain",      "cleric",    "War Domain",                  "Blessed for combat by a god of war.",               WAR_DOMAIN_AB));

        CLASSES.add(new ClassDefinition("druid",     "Druid",     "A guardian of nature and its cycles.",              DRUID_ATTRS,     DRUID_AB,     DRUID_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("circle_land",     "druid",     "Circle of the Land",          "Draws magical power from the terrain.",             CIRCLE_LAND_AB));
        SUBCLASSES.add(new SubclassDefinition("circle_moon",     "druid",     "Circle of the Moon",          "Shapeshifts into powerful beasts.",                 CIRCLE_MOON_AB));
        SUBCLASSES.add(new SubclassDefinition("circle_sea",      "druid",     "Circle of the Sea",           "Commands the power of wind and wave.",              CIRCLE_SEA_AB));
        SUBCLASSES.add(new SubclassDefinition("circle_stars",    "druid",     "Circle of the Stars",         "Reads power from the constellations.",              CIRCLE_STARS_AB));

        CLASSES.add(new ClassDefinition("fighter",   "Fighter",   "A versatile master of weaponry and armor.",         FIGHTER_ATTRS,   FIGHTER_AB,   FIGHTER_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("battle_master",   "fighter",   "Battle Master",               "Uses cunning maneuvers to control the battlefield.",BATTLE_MASTER_AB));
        SUBCLASSES.add(new SubclassDefinition("champion",        "fighter",   "Champion",                    "An elite warrior of pure physical excellence.",     CHAMPION_AB));
        SUBCLASSES.add(new SubclassDefinition("eldritch_knight", "fighter",   "Eldritch Knight",             "Combines martial training with arcane magic.",      ELDRITCH_KNIGHT_AB));
        SUBCLASSES.add(new SubclassDefinition("psi_warrior",     "fighter",   "Psi Warrior",                 "Augments combat with psionic power.",               PSI_WARRIOR_AB));

        CLASSES.add(new ClassDefinition("monk",      "Monk",      "A master of martial arts and ki energy.",           MONK_ATTRS,      MONK_AB,      MONK_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("warrior_mercy",   "monk",      "Warrior of Mercy",            "Heals allies and debilitates foes.",                WARRIOR_MERCY_AB));
        SUBCLASSES.add(new SubclassDefinition("warrior_shadow",  "monk",      "Warrior of Shadow",           "Blends combat and shadow magic.",                   WARRIOR_SHADOW_AB));
        SUBCLASSES.add(new SubclassDefinition("warrior_elements","monk",      "Warrior of the Elements",     "Channels elemental energy through strikes.",        WARRIOR_ELEMENTS_AB));
        SUBCLASSES.add(new SubclassDefinition("warrior_open_hand","monk",     "Warrior of the Open Hand",    "The purest expression of martial discipline.",      WARRIOR_OPEN_AB));

        CLASSES.add(new ClassDefinition("paladin",   "Paladin",   "A holy warrior bound by a sacred oath.",            PALADIN_ATTRS,   PALADIN_AB,   PALADIN_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("oath_devotion",   "paladin",   "Oath of Devotion",            "Upholds the ideal of a shining knight.",            OATH_DEVOTION_AB));
        SUBCLASSES.add(new SubclassDefinition("oath_glory",      "paladin",   "Oath of Glory",               "Strives to reach the heights of heroic legend.",    OATH_GLORY_AB));
        SUBCLASSES.add(new SubclassDefinition("oath_ancients",   "paladin",   "Oath of the Ancients",        "Preserves the light of life against darkness.",     OATH_ANCIENTS_AB));
        SUBCLASSES.add(new SubclassDefinition("oath_vengeance",  "paladin",   "Oath of Vengeance",           "Pursues and punishes evildoers relentlessly.",      OATH_VENGEANCE_AB));

        CLASSES.add(new ClassDefinition("ranger",    "Ranger",    "A skilled hunter of the wilderness.",               RANGER_ATTRS,    RANGER_AB,    RANGER_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("beast_master",    "ranger",    "Beast Master",                "Forms a deep bond with an animal companion.",       BEAST_MASTER_AB));
        SUBCLASSES.add(new SubclassDefinition("fey_wanderer",    "ranger",    "Fey Wanderer",                "Channels the magic of the Feywild.",                FEY_WANDERER_AB));
        SUBCLASSES.add(new SubclassDefinition("gloom_stalker",   "ranger",    "Gloom Stalker",               "Thrives in the darkest places of the world.",       GLOOM_STALKER_AB));
        SUBCLASSES.add(new SubclassDefinition("hunter",          "ranger",    "Hunter",                      "Specializes in tracking and slaying prey.",         HUNTER_AB));

        CLASSES.add(new ClassDefinition("rogue",     "Rogue",     "A cunning trickster who strikes from shadows.",     ROGUE_ATTRS,     ROGUE_AB,     ROGUE_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("arcane_trickster","rogue",     "Arcane Trickster",            "Enhances roguish talents with illusion magic.",     ARCANE_TRICKSTER_AB));
        SUBCLASSES.add(new SubclassDefinition("assassin",        "rogue",     "Assassin",                    "Trained to eliminate targets swiftly and silently.",ASSASSIN_AB));
        SUBCLASSES.add(new SubclassDefinition("soulknife",       "rogue",     "Soulknife",                   "Focuses psionic energy into blade-like constructs.",SOULKNIFE_AB));
        SUBCLASSES.add(new SubclassDefinition("thief",           "rogue",     "Thief",                       "Master of speed, stealth and sleight of hand.",     THIEF_AB));

        CLASSES.add(new ClassDefinition("sorcerer",  "Sorcerer",  "Born with innate magical power.",                   SORCERER_ATTRS,  SORCERER_AB,  SORCERER_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("draconic_sorcery","sorcerer",  "Draconic Sorcery",            "Dragon blood fuels your raw magical might.",        DRACONIC_AB));
        SUBCLASSES.add(new SubclassDefinition("wild_magic",      "sorcerer",  "Wild Magic",                  "Magic surges with chaotic unpredictable power.",    WILD_MAGIC_AB));
        SUBCLASSES.add(new SubclassDefinition("aberrant_sorcery","sorcerer",  "Aberrant Sorcery",            "Touched by alien forces from beyond reality.",      ABERRANT_AB));
        SUBCLASSES.add(new SubclassDefinition("clockwork_sorcery","sorcerer", "Clockwork Sorcery",           "Channels the ordered magic of clockwork planes.",   CLOCKWORK_AB));

        CLASSES.add(new ClassDefinition("warlock",   "Warlock",   "Gains power through a pact with a dark entity.",    WARLOCK_ATTRS,   WARLOCK_AB,   WARLOCK_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("archfey_patron",  "warlock",   "Archfey Patron",              "Pact with a powerful lord of the Feywild.",         ARCHFEY_AB));
        SUBCLASSES.add(new SubclassDefinition("fiend_patron",    "warlock",   "Fiend Patron",                "Pact made with a powerful devil for raw power.",    FIEND_AB));
        SUBCLASSES.add(new SubclassDefinition("great_old_one",   "warlock",   "Great Old One Patron",        "Bound to an unknowable cosmic entity.",             GREAT_OLD_ONE_AB));
        SUBCLASSES.add(new SubclassDefinition("celestial_patron","warlock",   "Celestial Patron",            "Pact with a being of the Upper Planes.",            CELESTIAL_AB));

        CLASSES.add(new ClassDefinition("wizard",    "Wizard",    "Master of studied spellcasting.",                   WIZARD_ATTRS,    WIZARD_AB,    WIZARD_ITEMS));
        SUBCLASSES.add(new SubclassDefinition("abjurer",         "wizard",    "Abjurer",                     "Specializes in protective and warding magic.",      ABJURER_AB));
        SUBCLASSES.add(new SubclassDefinition("diviner",         "wizard",    "Diviner",                     "Peers into the past, present and future.",          DIVINER_AB));
        SUBCLASSES.add(new SubclassDefinition("evoker",          "wizard",    "Evoker",                      "Channels raw elemental forces into destruction.",   EVOKER_AB));
        SUBCLASSES.add(new SubclassDefinition("illusionist",     "wizard",    "Illusionist",                 "Bends reality through weaves of illusion.",         ILLUSIONIST_AB));
    }
    ...
    public static List<SubclassDefinition> getSubclassesFor(String parentId) {
        ...
        for (SubclassDefinition s : SUBCLASSES)
            if (s.getParentClassId().equals(parentId)) out.add(s);
        return out;
    }

    public static ClassDefinition getClass(String id) {
        for (ClassDefinition c : CLASSES) if (c.getId().equals(id)) return c;
        return null;
    }

    public static SubclassDefinition getSubclass(String id) {
        for (SubclassDefinition s : SUBCLASSES) if (s.getId().equals(id)) return s;
        return null;
    }
}